...
namespace _6_FlowControl
{
  public class Program
  {
    ...
    public static int GetValidTemperature()
    {
      ...
      do
      {
        Console.WriteLine("Pleae type a number between -40 and 135 inclusive");
        string input = Console.ReadLine();
        unsuccessful = int.TryParse(input, out num);
      } while (!unsuccessful || num <= -40 || num > 135);
    ...
    /// <summary>
    /// This method has one int parameter.
    /// It checks if the int is <=42, Console.WriteLine($"{temp} is to`o cold!");
    ...
    public static void GetTemperatureTernary(int temp)
    {
      ...
      var resutl = temp <= 42 ? $"{temp} is too cold!" :
